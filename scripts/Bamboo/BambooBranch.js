...
function BambooBranch(x,y,maxx,maxy, parent) {
   	//initial variables
	this.x = x;
	this.y = y;
	...
	//relative
	if(parent.seed){
	        //random function aren't part of this object because apparently can't call until object is created?
		this.x_end = 0.1
		this.y_end = randomYBambooBranch(this.length) - 5;
	}else{
		this.x_end = randomXBambooBranch(this.length) ;
		this.y_end = randomYBambooBranch(this.length);
	}
	...
	this.grow = function(sprite){
		...
		if(this.age/5 < 1){
			...
			this.width = (this.age*2)/(1.5+this.distance_from_seed);
	...
	this.sprout_branch = function(branch_list, branch_sprite_list){
		if(((Math.random() * 100 + this.distance_from_seed ) < 40) || this.parent.seed){
			branch = new BambooBranch(this.endx,this.endy,this.max_x,this.max_y, this);
			...
			//only sprout leaf branch if you sprouted a branch
			this.sprout_leaf_branch(branch_list, branch_sprite_list);
		}//end if sprout
	}//end sprout branch
	
	//further from base you are, more likely to sprout leaf branches, don't sprout at all at very base
	this.sprout_leaf_branch = function(branch_list, branch_sprite_list){
		if(((Math.random() * 100 ) < 30 + this.distance_from_seed * 5) && this.distance_from_seed > 3){
	...
	this.tick = function(branch_list, branch_sprite_list, leaf_list, leaf_sprite_list, index){
		...
		//don't grow as the tree goes into stasis
			if(!this.is_stasis){
				...
				if(this.num_branches < 1 && branch_list.length < this.max_branches ){
...
//want it to be almost straight, but not quite
function randomXBambooBranch(length){
...
function randomYBambooBranch(length){
