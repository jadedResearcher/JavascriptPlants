//a bush branch is shorter, thiner, and well...bushier.

//has a start point (x and y)
//and an end point
//it randomly creates a mostly straight path from start to end
function BushBranch(x,y,maxx,maxy, parent) {
   	//initial variables
	this.x = x;
	this.y = y;
	...
	this.grow = function(sprite){
		...
		if(this.age/5 < 1){
			...
			this.width = this.age/(5+this.distance_from_seed);
		...
		this.y_end = this.y_end + (this.y_end/100) * 2 + added_length;
	    this.endx = this.x + this.x_end;
	    this.endy = this.y + this.y_end;
		this.set_path();
	...
	this.sprout_branch = function(branch_list, branch_sprite_list){
		if(((Math.random() * 100 + this.distance_from_seed * 2) < 30) || this.parent.seed){
			branch = new BushBranch(this.endx,this.endy,this.max_x,this.max_y, this);
	...
	this.sprout_leaf_berry = function(leaf_list, leaf_sprite_list){
		if((Math.random() * 1000000 - (this.max_leaves - this.num_leaves)  < 40 - this.width*20)|| this.is_stasis && this.width < 2){
			if(Math.random() * 100 < 60){
				...
				//sprout berry
				berry = new Berry(this.endx,this.endy,this.max_x,this.max_y, this);
				leaf_list.push(berry);
				berry_sprite = paper.path(berry.path);
				leaf_sprite_list.push(berry_sprite);
				leaf_sprite_list[leaf_sprite_list.length - 1].attr({fill: berry.color, stroke: berry.stroke});
				this.children.push(berry);
				this.num_children = this.children.length;
				this.num_leaves = this.num_leaves + 1;
			}
	...
	this.tick = function(branch_list, branch_sprite_list, leaf_list, leaf_sprite_list, index){
		...
		//don't grow as the tree goes into stasis
			if(!this.is_stasis){
				...
				if(this.num_branches < 3 && branch_list.length < this.max_branches ){
		...
		if((this.num_leaves < 2 && leaf_list.length < this.max_leaves) || this.is_stasis){
			this.sprout_leaf_berry(leaf_list, leaf_sprite_list);
		}
