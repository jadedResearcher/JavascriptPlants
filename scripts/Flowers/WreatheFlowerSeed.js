...
function WreatheFlowerSeed(x,y,maxx,maxy, scale, red, green, blue) {
   	//initial variables
	this.x = x;
	this.wreathe = true;
	this.y = y;
	...
	if(red){
		...
		this.red_petal = Math.random() * 255;
	}
	
	if(blue){
		this.blue_petal = blue;
	}else{
		this.blue_petal = Math.random() * 255;
	}
	
	if(green){
		this.green_petal = green;
	}else{
		this.green_petal = Math.random() * 100;
	}
	this.stroke = '#000';
	//test method
	this.test = function(){
