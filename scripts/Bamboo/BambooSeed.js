...
function BambooSeed(x,y,maxx,maxy) {
   	//initial variables
	this.x = x;
	this.y = y;
	this.endx = x;
	...
	this.max_x = maxx;
	this.max_y = maxy;
	this.num_seed_children = 0;
	//decides the main color of the bamboo
	this.red = Math.floor(Math.random() * 100) + 55;
	this.green = Math.floor(Math.random() * 100) + 100;
	...
	this.tick = function(root_list, root_sprite_list, branch_list, branch_sprite_list){
		...
		if (this.num_branches == 0 && this.age > 5){
		    ...
		    branch = new BambooBranch(this.x,this.y,this.max_x,this.max_y, this);
